    ...
    <main class="layout-container flex h-full grow flex-col w-full max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div class="flex flex-col md:flex-row flex-wrap justify-between gap-6 mb-8 items-end md:items-center">
            ...
            <div class="flex gap-3">
                ...
                @endif
                <button onclick="{{ auth()->user()->subscribed() ? '' : 'showUpgradeModal()' }}" class="flex items-center justify-center gap-2 rounded-lg h-10 px-5 {{ auth()->user()->subscribed() ? 'bg-primary' : 'bg-gray-200 dark:bg-gray-700' }} hover:bg-primary-dark text-text-main text-sm font-bold transition-colors shadow-lg shadow-primary/20">
                    <span class="material-symbols-outlined text-lg">{{ auth()->user()->subscribed() ? 'table_view' : 'lock' }}</span>
                            ...
                            <div class="flex flex-col gap-4">
                                ...
                                <div class="flex flex-col gap-3">
                                    <div class="grid grid-cols-1 md:grid-cols-3 gap-4 items-center px-2 py-1 hover:bg-white dark:hover:bg-card-dark rounded-lg transition-colors">
                                        ...
                                        <span class="text-sm font-bold text-primary md:text-right {{ auth()->user()->subscribed() ? '' : 'blur-[4px] select-none' }}">RM {{ number_format($projEmployment, 2) }}</span>
                                    </div>
                                    <div class="grid grid-cols-1 md:grid-cols-3 gap-4 items-center px-2 py-1 hover:bg-white dark:hover:bg-card-dark rounded-lg transition-colors">
                                        ...
                                        <span class="text-sm font-bold text-primary md:text-right {{ auth()->user()->subscribed() ? '' : 'blur-[4px] select-none' }}">RM {{ number_format($projRental, 2) }}</span>
                                    </div>
                                    <div class="grid grid-cols-1 md:grid-cols-3 gap-4 items-center px-2 py-1 hover:bg-white dark:hover:bg-card-dark rounded-lg transition-colors">
                                        ...
                                        <span class="text-sm font-bold text-primary md:text-right {{ auth()->user()->subscribed() ? '' : 'blur-[4px] select-none' }}">RM {{ number_format($projOther, 2) }}</span>
                                    </div>
            ...
            <div class="flex flex-col gap-6">
                ...
                <div class="rounded-xl bg-gradient-to-br from-[#102219] to-[#1e3b2e] dark:from-black dark:to-[#0a150f] p-6 text-white relative overflow-hidden">
                    ...
                    <div class="relative z-10 flex flex-col gap-3">
                        ...
                        <button onclick="{{ auth()->user()->subscribed() ? '' : 'showUpgradeModal()' }}" class="mt-2 w-full py-2 bg-white/10 hover:bg-white/20 border border-white/20 rounded-lg text-sm font-semibold transition-colors">
                            {{ auth()->user()->subscribed() ? 'Upload Receipt' : 'Unlock Receipt Upload' }}
                        </button>
                    </div>
                </div>
            </div>
