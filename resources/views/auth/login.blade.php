...
<style>
.checkbox-tick-svg {
    background-image: url(https://lh3.googleusercontent.com/aida-public/AB6AXuDnKVQsqnuaxq9o9qP8l3ziT8iRA-3YARWNe-SOsAJtzf-VknC-m1wsPe0qlwQcRiVQLWGpJ3zPZ--vOqDgli7NmJ5GkXrmt5QM5Q7w-fPd--69bQEqY9pHsr-dWQ1h7o_rAvMBFwBSv-tvr0u_GPBlt0WqkS6QrjNM5jQ9w89yjZnjsOpn1HcWKUZAX8aD2g_W99-EDYoTfo-18fW20WfwF-t8ftfe8kiamvDgJyBONkMNMwiepqX-z-xFgasJtyCYekUGm23LmA0)
}
</style>
</head>
...
<img class="w-full h-full object-cover" src="https://lh3.googleusercontent.com/aida-public/AB6AXuD35mky1Obg7SOkImSIJydoKv8vr_8FDkLCSmA6C-qZ00rJb-83mVeJeTd2oFC1HWA7-lDxPUZkqzpTNo1dKQkgEQoLbAf2A_0WCSIEw1HubQgDjsQHBIiNM3ZNoAElcbWpE3dZXpoaYugx1zFstDYNdeW-rZmUIERncFxg3StlFCT_8d-W80ldG6lzxE12ie9aSTaOBvGgdPr7rZIyZf7pmTsDPH8--BHP2WRubpmEmtVj6f31MXwjCrhH7eogdFpU8E9kM55f-vw" alt="Office background"/>
...
<button class="group flex flex-col items-center justify-center border-b-[3px] border-b-text-main dark:border-b-primary pb-[13px] pt-4 px-2 outline-none focus:outline-none">
...
<a href="{{ route('register') }}" class="group flex flex-col items-center justify-center border-b-[3px] border-b-transparent pb-[13px] pt-4 px-2 outline-none focus:outline-none hover:border-b-border-color">
...
<input name="email" value="{{ old('email') }}" class="form-input w-full rounded-lg border border-border-color dark:border-gray-600 bg-white dark:bg-gray-800 text-text-main dark:text-white h-12 px-4 placeholder:text-text-muted dark:placeholder:text-gray-500 focus:border-primary focus:ring-1 focus:ring-primary focus:outline-none transition-all @error('email') border-red-500 @enderror" placeholder="name@company.com" type="email" required autofocus/>
@error('email')
...
<input name="password" class="form-input w-full rounded-lg border border-border-color dark:border-gray-600 bg-white dark:bg-gray-800 text-text-main dark:text-white h-12 pl-4 pr-12 placeholder:text-text-muted dark:placeholder:text-gray-500 focus:border-primary focus:ring-1 focus:ring-primary focus:outline-none transition-all @error('password') border-red-500 @enderror" placeholder="Enter your password" type="password" required autocomplete="current-password"/>
...
<input name="remember" class="h-5 w-5 rounded border-border-color dark:border-gray-600 border-2 bg-transparent text-primary checked:bg-primary checked:border-primary checkbox-tick-svg focus:ring-0 focus:ring-offset-0 focus:outline-none transition-colors cursor-pointer" id="remember-me" type="checkbox"/>
...
<!-- Divider and Social Login omitted for brevity if not functional, but keeping them for theme consistency -->
